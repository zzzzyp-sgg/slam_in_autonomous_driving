...
void LoamLikeOdom::ProcessPointCloud(FullCloudPtr cloud) {
    ...
    if (current_edge->size() < options_.min_edge_pts_ || current_surf->size() < options_.min_surf_pts_ || current_ground->size() < options_.min_ground_pts_) {
        LOG(ERROR) << "not enough edge/surf pts: " << current_edge->size() << "," << current_surf->size() << "," << current_surf->size();
        return;
    }
    ...
    if (IsKeyframe(pose)) {
        ...
        if (grounds_.size() > options_.num_kfs_in_local_map_) { // 【新增】
            grounds_.pop_front();
        }
